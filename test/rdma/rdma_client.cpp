...
int main() {
    ...
    // pin memory
    // create queue pair
    assert( resources_create(&res) == 0);

    assert( connect_qp(&res) == 0);

    char temp_char;
    assert( sock_sync_data(res.sock, 1, "R", &temp_char) == 0);
    ...
    assert( sock_sync_data (res.sock, 1, "W", &temp_char) == 0);
